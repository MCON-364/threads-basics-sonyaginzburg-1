...
public class ParallelSumCalculator {
    ...
    public long parallelSum(List<Integer> numbers, int workers)
        ...
        int slices = (numbers.size() + workers - 1 )/ workers;
        ...
        IntStream.iterate(0, start -> start < numbers.size(), start -> start + slices)
                .forEach(start -> {
                    List<Integer> slice = numbers.subList(start, Math.min(start + slices, numbers.size()));
